...
for t=1:numel(k)
    ...
    %plot3(path(:,2),path(:,1),path(:,3),'ro','MarkerSize', 3,'Color','red')
    %[x1,y1,z1] = sphere;
    for i=1:MaxIt
        %x=x1*rs(t);
        %y=y1*rs(t);
        ...
        if i > 1
            plot3([path(i,2) path(i-1,2)], [path(i,1) path(i-1,1)], [path(i,3) path(i-1,3)], 'green', 'LineWidth', 1)
        end
